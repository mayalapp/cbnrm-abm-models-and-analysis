...
## Vallino model corrected 

There is one really weird outlier of "Corrected" C&E model under base paramteres? See row # 64. Don't know what happened there.. 
```{r}

baseData_vallinoCorrected[60:65,]

baseData_vallinoCorrected%>%ggplot(aes(x= K, y = TotalBM))+
  geom_point()

```
```{r}
baseData_vallinoCorrected%>%summarize(meanBM = mean(TotalBM), meanNumLog = mean(numLoggers))

baseData_vallinoCorrected%>%ggplot(aes(x = numLoggers*100))+
  geom_histogram()+
  xlab("% of Loggers remaining")
baseData_vallinoCorrected%>%ggplot(aes(x = numLoggers*100))+
...
Kf vs BM scatter plot - corrected C&E model 
```{r }
# make variable to color code exceptions to Kf = 0 
