...
ggsave("./figs/viz_monSanc.2.8.pdf", viz_monSanc_2_8, width=5.75, height=6, units="in")

```
surveillance doesn't matter 
```{r}
enfGraph=enfData_vallino%>%mutate(enfValue=rev(as.factor((as.numeric(variableValue)-1)/10)))
enfGraph%>% filter(enfValue == 0.2|enfValue == 0.8)%>%ggplot(aes(x = numLoggers, y = TotalBM*100))+
  geom_point(aes(color = enfValue))+
  labs(color = "surveillance-level")
```
...
test_mean = mean(test_deviation$TotalBM)
test_sd = sd(test_deviation$TotalBM)
test_mean
test_sd

test_deviation%>%ggplot(aes(x = TotalBM))+
  geom_histogram()+
  geom_vline(xintercept = test_mean)
