...
simExportPlots <- function(params) {
  ...
  for(i in 1:runs){
    ...
    NLCommand("export-all-plots \"exportPlots_mon.csv\"") 
    output[[i]]<-getGraphs("exportPlots_mon.csv") 
...
paramVals <- (0:10)*10
dataMonitoring<- runParams(paramVals, base.param, "myMod_revised_changeMonitoring.RData", "monitoring-level")
