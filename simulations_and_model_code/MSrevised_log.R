...
simExportPlots <- function(params) {
  ...
  for(i in 1:runs){
    ...
    NLCommand("export-all-plots \"exportPlots_log.csv\"") 
    output[[i]]<-getGraphs("exportPlots_log.csv") 
...
dataLog<- runParams(paramVals, base.param, "myMod_revised_changeLog.RData", "initial-loggers")
