...
simExportPlots <- function(params) {
  ...
  for(i in 1:runs){
    ...
    NLCommand("export-all-plots \"exportPlots_cheat.csv\"") 
    output[[i]]<-getGraphs("exportPlots_cheat.csv") 
...
paramVals <- (0:10)/10
dataCheat<- runParams(paramVals, base.param, "myMod_revised_changeCheat.RData", "initial-prob-cheat")
