...
simExportPlots <- function(params) {
  ...
  for(i in 1:runs){
    ...
    NLCommand("export-all-plots \"exportPlots_rt.csv\"") 
    output[[i]]<-getGraphs("exportPlots_rt.csv") 
...
# reference threshold 
paramVals <- (1:10)/10
dataRT<- runParams(paramVals, base.param, "myMod_revised_changeRT.RData", "reference-threshold")
